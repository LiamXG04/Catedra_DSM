...
class GolosinasActivity : AppCompatActivity() {
    ...
    private fun loadCandies() {
        ...
        lifecycleScope.launch {
            try {
                val response = RetrofitClient.instance.getCandies()

                if (response.isSuccessful) {
                    val candies = response.body() ?: emptyList()
                    val candyItems = candies.map { CandyAdapter.CandyItem(it) }.toMutableList()

                    candyAdapter = CandyAdapter(candyItems) { newSubtotal ->
                    ...
                    updateSubtotal(0.0) // Inicializar subtotal en 0
                } else {
